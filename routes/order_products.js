...
} = require("../db/adapters/order_products");
const { getOrderById } = require("../db/adapters/orders");

// If modifying data, make sure the user is logged in and the correct user
    ...
    try {
      ...
      if (req.user.id === order.user_id) {
        if (!orderProduct) {
          ...
          const updatedOrderProduct = await updateOrderProducts(
            orderProductId,
            quantity
          );
          res.send({
  ...
  async (req, res, next) => {
    ...
    if (req.user.id === order.user_id) {
      if (!orderProduct) {
        ...
        const destroyedOrderProduct = await destroyOrderProducts(
          orderProductId
        );
        res.send({
